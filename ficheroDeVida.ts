...
var pais: any; //variable que toma nombre del país seleccionado
let divContenido = document.querySelector("#contenido");
(divContenido as HTMLDivElement).innerHTML = "";

let limpiar = (elemento: HTMLDivElement) => {//función para limpiar el "select"
  elemento.innerHTML = "";
};

document.querySelector("#continente")?.addEventListener("change", () => {
  (document.querySelector("#paises") as HTMLSelectElement).innerHTML="";
  ...
  if (value === "0") {
    ...
    (h1 as HTMLHeadingElement).innerHTML = "Debe seleccionar un continente";
  ...
  paises
    .getDatos(
      "https://restcountries.com/v3.1/name/" +
        (selectPaises as HTMLSelectElement).selectedOptions[0].value
    )
    .then((datos: Array<any>) => {
      datos.forEach(
        (info: {
          ...
          (
            parrafo1 as HTMLParagraphElement
          ).innerHTML = `Área: ${areaEnMiles} km²`;
    ...
    .then((datos: Array<any>) => {
      datos.forEach(
        (info: {
          ...
          (
            parrafo1 as HTMLParagraphElement
          ).innerHTML = `Nombre común: ${info.name.common}`;
          (
            parrafo2 as HTMLParagraphElement
          ).innerHTML = `Nombre oficial: ${info.name.official}`;
          (
            parrafo3 as HTMLParagraphElement
          ).innerHTML = `Capital: ${info.capital}`;
          (
            parrafo4 as HTMLParagraphElement
          ).innerHTML = `Población: ${populationEnMiles} habitantes`;
